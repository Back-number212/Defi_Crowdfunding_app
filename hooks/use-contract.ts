...
export function useContract() {
  ...
  const createCampaign = useCallback(async (params: CreateCampaignParams) => {
    ...
    try {
      ...
      // Validate and prepare reward tiers (these are added separately after campaign creation)
      ...
      // Prepare arrays for reward tiers
      const tierNames: string[] = []
      const tierAmounts: bigint[] = []
      
      for (const tier of rewardTiers) {
        tierNames.push(tier.description)
        tierAmounts.push(tier.requiredAmount)
      }
      
      console.log("Calling contract.createCampaign with raw arguments:")
      console.log("  title:", params.title, " (type:", typeof params.title, ")")
      console.log("  description:", params.description, " (type:", typeof params.description, ")")
      console.log("  image:", (params.image || ""), " (type:", typeof (params.image || ""), ")")
      console.log("  targetWei:", targetWei, " (type:", typeof targetWei, ")")
      console.log("  durationInDays:", durationInDays, " (type:", typeof durationInDays, ")")
      console.log("  categoryIndex:", categoryIndex, " (type:", typeof categoryIndex, ")")
      console.log("  tierNames:", tierNames)
      console.log("  tierAmounts:", tierAmounts)
      
      // Create campaign with reward tiers in a single transaction
    ...
    try {
      ...
      if (campaignState !== 0) {
        ...
        if (campaignState === 1) {
          ...
          stateMessage = `Chiến dịch đang ở trạng thái ${stateNames[campaignState] || `State ${campaignState}`}. Bạn không thể quyên góp.`
        }
      ...
      if (err.reason) {
        ...
        if (err.message.includes("Campaign is not active")) {
          ...
          errorMessage = "Quyên góp thất bại. Vui lòng kiểm tra: 1) Chiến dịch đang hoạt động, 2) Số tiền > 0, 3) Bạn có đủ số dư"
        }
      }
  ...
  const getRefund = useCallback(async (campaignId: number) => {
    ...
    try {
      console.log("Attempting to get refund for campaign:", campaignId);
      ...
      console.log("Campaign state:", Number(campaign.state))
      
      if (Number(campaign.state) !== 2) { // 2 = Failed
  ...
  const getUserContribution = useCallback(async (campaignId: number): Promise<bigint> => {
    ...
    try {
      const contribution = await contract.getUserContribution(campaignId, account)
