...
export function CampaignDetailModal({ 
  ...
  if (isOpen) {
    console.log("Campaign Detail Modal - Full Campaign Object:", campaign);
    ...
    console.log("Refund Button State Debug:", {
      isExpired: isExpired,
      goalReached: goalReached,
      isConnected: isConnected,
      isCorrectNetwork: isCorrectNetwork,
      campaignState: Number(campaign.state)
    })
  }
  ...
  const handleDonate = async () => {
    ...
    if (campaignState !== 0) {
      ...
      let title = ""
      if (campaignState === 1) {
        title = "Chiến dịch đã thành công"
        message = "Chiến dịch đã đạt mục tiêu và không còn nhận quyên góp."
      } else if (campaignState === 2) {
        title = "Chiến dịch đã kết thúc"
        message = "Chiến dịch đã kết thúc và không đạt mục tiêu. Bạn không thể quyên góp nữa."
      } else if (campaignState === 3) {
        title = "Chiến dịch đã được rút tiền"
        message = "Chiến dịch đã được rút tiền. Bạn không thể quyên góp nữa."
      } else {
        title = "Không thể quyên góp"
        message = "Chiến dịch không còn ở trạng thái hoạt động."
      }
    ...
    try {
      ...
      if (result) {
        ...
        toast({
          ...
          description: "Cảm ơn bạn đã đóng góp cho chiến dịch này.",
          variant: "default"
        })
        onClose()
      } else {
        // Error message is already set by donateToCampaign hook
        const errorMessage = error || "Vui lòng kiểm tra kết nối ví và thử lại."
        toast({
          title: "Quyên góp thất bại",
          description: errorMessage,
          variant: "destructive"
        })
        // Clear error after showing toast to prevent Alert from showing
      ...
      toast({
        title: "Quyên góp thất bại",
        description: error?.message || "Vui lòng kiểm tra kết nối ví và thử lại.",
        variant: "destructive"
      })
    }
  }
    ...
    try {
      ...
      if (result) {
        ...
        toast({
          ...
          description: "Tiền đã được chuyển vào ví của bạn.",
          variant: "default"
        })
        onClose()
      } else {
  ...
  const handleRefund = async () => {
    // Check campaign state before attempting refund
    const campaignState = Number(campaign.state)
    if (campaignState !== 2) {
      let message = ""
      if (campaignState === 0) {
        message = "Chiến dịch vẫn đang hoạt động. Bạn chỉ có thể hoàn tiền khi chiến dịch thất bại."
      } else if (campaignState === 1) {
        message = "Chiến dịch đã thành công. Bạn không thể hoàn tiền."
      } else if (campaignState === 3) {
        message = "Chiến dịch đã được rút tiền. Bạn không thể hoàn tiền."
      } else {
        message = "Chiến dịch không ở trạng thái thất bại. Bạn chỉ có thể hoàn tiền khi chiến dịch thất bại."
      }
    ...
    try {
      ...
      if (result) {
        ...
        toast({
          ...
          description: "Tiền đã được hoàn lại vào ví của bạn.",
          variant: "default"
        })
        onClose()
      } else {
  ...
  const handleStopCampaign = async () => {
    // Check campaign state before attempting to stop
    const campaignState = Number(campaign.state)
    if (campaignState !== 0) {
      let message = ""
      if (campaignState === 1) {
        message = "Chiến dịch đã thành công. Không thể dừng."
      } else if (campaignState === 2) {
        message = "Chiến dịch đã thất bại. Không thể dừng."
      } else if (campaignState === 3) {
        message = "Chiến dịch đã được rút tiền. Không thể dừng."
      } else {
        message = "Chiến dịch không ở trạng thái hoạt động. Chỉ có thể dừng các chiến dịch đang hoạt động."
      }
      
      toast({
        title: "Không thể dừng chiến dịch",
        description: message,
        variant: "destructive"
      })
      clearError()
    ...
    try {
      if (confirm("Bạn có chắc chắn muốn dừng chiến dịch này? Hành động này không thể hoàn tác.")) {
        clearError()
        const result = await stopCampaign(campaign.id)
        if (result) {
          clearError()
          toast({
            title: "Dừng chiến dịch thành công!",
            description: "Chiến dịch đã được dừng.",
            variant: "default"
          })
          onClose()
        } else {
          const errorMessage = error || "Vui lòng kiểm tra kết nối ví và thử lại."
          toast({
            title: "Dừng chiến dịch thất bại",
            description: errorMessage,
            variant: "destructive"
          })
          setTimeout(() => clearError(), 100)
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Button
              ...
              disabled={Number(campaign.state) !== 2 || !isConnected || !isCorrectNetwork}
              className="bg-gradient-to-r from-blue-600 to-purple-600 text-white shadow-lg hover:opacity-90 transition-all duration-200"
            >
              Get Refund
            </Button>
            ...
            <Tabs defaultValue="updates" className="w-full">
              ...
              <TabsContent value="updates" className="mt-4">
                <CampaignUpdates
                  ...
                  isCreator={isOwner || actualIsOwner}
                />
              </TabsContent>
