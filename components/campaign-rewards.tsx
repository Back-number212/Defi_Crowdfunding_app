...
export function CampaignRewards({
  ...
  const { getCampaignTiers, getUserContribution } = useContract()
  ...
  // Load total contribution for connected user
  useEffect(() => {
    if (isConnected && account && rewardTiers.length > 0) {
      loadTotalContribution()
    }
  }, [campaignId, account, isConnected, rewardTiers.length, getUserContribution])
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {isHighestEligible && !isHighestQualified && donationAmount && (
                          <Badge variant="default" className="bg-green-500 text-white text-xs">
                            <CheckCircle2 className="h-3 w-3 mr-1" />
                            You qualify!
                          </Badge>
                        )}
                        {isEligible && !isHighestEligible && !isHighestQualified && donationAmount && (
